...
This specification defines the `model.json` payload consumed by `core-structural`. Version **v0.1** is millimeter-only; all coordinates and dimensions are expressed in **mm**, and linear weight is expressed in **kg/m**.
...
- `orientation` — Optional; `rollAngleDeg` in degrees, `camberMm` in millimeters.
...
- `plateIds` — Optional plate references (empty if not used).
