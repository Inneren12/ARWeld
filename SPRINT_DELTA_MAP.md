...
| **S2-16** | pipeline детекции маркеров: markerId + 4 угла кадра | ✅ IMPLEMENTED | `feature-arview/src/main/kotlin/.../marker/MarkerDetector.kt`<br>`feature-arview/src/main/kotlin/.../marker/RealMarkerDetector.kt` | ML Kit barcode detector on ARCore frames returns markerId (rawValue) + ordered corners in camera image coords with throttling; simulated detector kept only for manual debug |
