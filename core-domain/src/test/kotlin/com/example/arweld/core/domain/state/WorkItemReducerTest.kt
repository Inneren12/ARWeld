...
class WorkItemReducerTest {
    ...
    fun `simple claim sets in progress and assignee`() {
        val claimEvent = event(EventType.WORK_CLAIMED, actorId = "user-1", timestamp = 1L)
    ...
    fun `happy path progresses to approved`() {
        val claim = event(EventType.WORK_CLAIMED, actorId = "user-1", timestamp = 1L)
        val readyForQc = event(EventType.WORK_READY_FOR_QC, actorId = "user-1", timestamp = 2L)
        val qcStarted = event(EventType.QC_STARTED, actorId = "qc-1", timestamp = 3L)
        val qcPassed = event(EventType.QC_PASSED, actorId = "qc-1", timestamp = 4L)
    ...
    fun `rework path flags rework required`() {
        val events = listOf(
            event(EventType.WORK_CLAIMED, actorId = "user-1", timestamp = 1L),
            event(EventType.WORK_READY_FOR_QC, actorId = "user-1", timestamp = 2L),
            event(EventType.QC_STARTED, actorId = "qc-1", timestamp = 3L),
            event(EventType.QC_FAILED, actorId = "qc-1", timestamp = 4L),
        )
    ...
    fun `unsorted events are processed chronologically`() {
        val qcPassed = event(EventType.QC_PASSED, actorId = "qc-1", timestamp = 40L)
        val qcStarted = event(EventType.QC_STARTED, actorId = "qc-1", timestamp = 30L)
        val readyForQc = event(EventType.WORK_READY_FOR_QC, actorId = "user-1", timestamp = 20L)
        val claimed = event(EventType.WORK_CLAIMED, actorId = "user-1", timestamp = 10L)
